...
async function extractTextFromWordBuffer(fileObject) { // Takes the multer file object
  try {
    console.log(`Attempting to extract text from Word buffer: ${fileObject.originalname}`);
    ...
    console.error(`Error extracting text from Word buffer (${fileObject.originalname}):`, error);
    throw error;
  }
}

async function extractTextFromPDFBuffer(fileObject) { // Takes the multer file object
  try {
    console.log(`Attempting to extract text from PDF buffer: ${fileObject.originalname}`);
    if (!fileObject.buffer) {
    ...
    console.error(`Error extracting text from PDF buffer (${fileObject.originalname}):`, error);
    throw error;
  }
}

async function extractTextFromTextBuffer(fileObject) { // Takes the multer file object
    try {
      console.log(`Attempting to extract text from TXT buffer: ${fileObject.originalname}`);
      if (!fileObject.buffer) {
      ...
      console.error(`Error extracting text from TXT buffer (${fileObject.originalname}):`, error);
      throw error;
    }
  }
...
async function extractTextFromBuffer(fileObject) {
  if (!fileObject || !fileObject.buffer || !fileObject.mimetype || !fileObject.originalname) {
    throw new Error(`Invalid file object provided for buffer extraction. Received: ${JSON.stringify(fileObject)}`);
  }
  try {
    console.log(
...
`;
  ...
  try {
    ...
    try {
      ...
      console.error("Failed to parse JSON from Gemini response:", parseError, "Raw Response:", responseText);
      const jsonMatch = responseText.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
         try {
            jsonResponse = JSON.parse(jsonMatch[0]);
            if ( /* re-validate structure */
                typeof jsonResponse.name !== 'string' || typeof jsonResponse.email !== 'string' ||
                typeof jsonResponse.similarityScore !== 'number' || !Array.isArray(jsonResponse.commonSkills) ||
                typeof jsonResponse.preferred !== 'string' || typeof jsonResponse.roleName !== 'string' ||
                typeof jsonResponse.mainDomain !== 'string'
              ) { throw new Error("Parsed substring JSON has unexpected structure."); }
         } catch (substringParseError) {
            console.error("Failed to parse JSON substring:", substringParseError);
            throw new Error("Could not parse valid JSON from Gemini response (substring attempt).");
         }
      } else {
    ...
    console.error("Gemini API Error during comparison or final processing:", error);
    return {
      name: "N/A", email: "N/A", phone: "N/A", location: "N/A",
      keySkills: [], totalExperienceYears: 0, mainDomain: "N/A",
      companiesWorked: [], education: [], similarityScore: 0,
      commonSkills: [], preferred: "No", roleName: "N/A (Error)",
