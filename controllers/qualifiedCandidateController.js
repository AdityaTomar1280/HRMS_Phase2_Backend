...
exports.saveSelectedCandidates = async (req, res) => {
  try {
    ...
    const result = await QualifiedCandidate.bulkWrite(operations);
...
exports.getAllCandidates = async (req, res) => {
  try {
    ...
    const candidates = await QualifiedCandidate.find()
      .sort({ updatedAt: -1 })
...
exports.getCandidateById = async (req, res) => {
  try {
    ...
    const candidate = await QualifiedCandidate.findById(id).select("-resumeFile.data");
...
exports.getCandidateResume = async (req, res) => {
  try {
    ...
    const candidate = await QualifiedCandidate.findById(id).select("resumeFile");
...
exports.storeResumeFile = async (req, res) => {
  try {
    ...
    const candidate = await QualifiedCandidate.findById(id);
...
exports.deleteCandidate = async (req, res) => {
  try {
    ...
    const result = await QualifiedCandidate.findByIdAndDelete(id);
...
exports.getInterviewTrackingData = async (req, res) => {
  try {
    ...
    const stageCounts = await QualifiedCandidate.aggregate([
      { $group: { _id: "$interviewStatus", count: { $sum: 1 } } },
      { $sort: { _id: 1 } },
    ]);

    // Get candidates with upcoming interviews
    const upcomingInterviews = await QualifiedCandidate.find({
    ...
    const recentlyUpdated = await QualifiedCandidate.find()
      .sort({ updatedAt: -1 })
      .limit(5)
      .select("-resumeFile.data");
